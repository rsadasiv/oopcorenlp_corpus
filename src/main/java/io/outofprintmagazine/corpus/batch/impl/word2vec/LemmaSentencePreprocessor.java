...
public class LemmaSentencePreprocessor extends CorpusBatchStep implements CorpusBatchStepTask {

	private static final Logger logger = LogManager.getLogger(LemmaSentencePreprocessor.class);
		
	public LemmaSentencePreprocessor() {
		super();
	}
	...
	public void enrichOne(ObjectNode outputStepItem) throws Exception {
		...
		outputStepItem.put(
				"word2vec_LemmaSentence_Preprocessed",
				getStorage().storeScratchFileString(
					getData().getCorpusId(),
					getOutputScratchFilePath("LemmaSentence_" + getDocID(outputStepItem), "txt"),
					txt.toString()
			)
		);
	}
