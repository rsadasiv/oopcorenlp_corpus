...
public class LemmaParagraphPreprocessor extends CorpusBatchStep implements CorpusBatchStepTask {

	private static final Logger logger = LogManager.getLogger(LemmaParagraphPreprocessor.class);
		
	public LemmaParagraphPreprocessor() {
		super();
	}
	...
	public void enrichOne(ObjectNode outputStepItem) throws Exception {
		...
		outputStepItem.put(
				"word2vec_LemmaParagraph_Preprocessed",
				getStorage().storeScratchFileString(
					getData().getCorpusId(),
					getOutputScratchFilePath("LemmaParagraph_" + getDocID(outputStepItem), "txt"),
					txt.toString()
			)
		);
	}
